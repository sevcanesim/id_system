...
export async function GET(request: NextRequest) {
  ...
  const [{ data: variants, error: variantError }, { data: plans, error: planError }] = await Promise.all([
    ctx.admin.from("product_variants").select("id,sku,name,price_kurus,billing_period,metadata,is_active").in("sku", [...PRODUCT_SKUS]).order("sku", { ascending: true }),
...
export async function PATCH(request: NextRequest) {
  ...
  if (parsed.data.kind === "PRODUCT_VARIANT") {
    ...
    const { data: after, error } = await ctx.admin.from("product_variants").update({ price_kurus: parsed.data.priceKurus }).eq("sku", parsed.data.sku).select("id,sku,name,price_kurus").maybeSingle();
  ...
  const { error: variantError } = await ctx.admin.from("product_variants").update({ price_kurus: parsed.data.priceKurus }).eq("sku", sku);
  if (variantError) {
