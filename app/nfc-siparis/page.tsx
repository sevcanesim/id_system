...
export default function NfcOrderPage() {
  ...
  if (!isSupabaseConfigured || !userId) return <main className="order-page order-page-v166"><section className="result-empty"><span className="section-kicker">GÜVENLİ SİPARİŞ</span><h1>Sipariş için hesabına giriş yap.</h1><p>Kartını kişisel Yenomi ID profiline güvenle bağlamak için oturum açmalısın.</p><Link href="/giris?next=%2Fnfc-siparis">Giriş Yap</Link></section></main>;
  if (completed) return <main className="order-page"><section className="order-success"><span>✓</span><h1>Sipariş talebin alındı.</h1><p>Üretim ayrıntıları ve ödeme için seninle iletişime geçeceğiz.</p><div className="order-success-actions"><Link href="/siparislerim">Siparişimi Takip Et</Link><Link className="secondary" href="/kartim">Kartvizitime Dön</Link></div></section></main>;
