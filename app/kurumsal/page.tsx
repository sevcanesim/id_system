    ...
    <main id="main-content" className="theme-light corporate-page corporate-sales-page corporate-single-page">
      <section className="corporate-single-hero corporate-authentic-hero" aria-labelledby="corporate-single-title">
        <div className="corporate-single-copy">
          ...
          <div className="corporate-hero-actions">
            <a href="#kapasite" className="corporate-cta">Paketleri Gör <span aria-hidden="true">→</span></a>
        ...
        <aside className="corporate-authentic-proof corporate-hero-specimen" aria-label="Yenomi ID kurumsal yönetim ekranı örneği">
          ...
          <div className="corporate-specimen-table">
            <div className="corporate-specimen-table__header"><span>Çalışan</span><span>Ünvan</span><span>Durum</span><span>Aksiyon</span></div>
            {employeeSpecimens.map((emp) => <div key={emp.name} className="corporate-specimen-table__row"><div className="corporate-specimen-user"><span className="corporate-specimen-avatar">{emp.name[0]}</span><strong>{emp.name}</strong></div><span className="corporate-specimen-role">{emp.role}</span><span className={`corporate-specimen-status ${emp.statusClass}`}>{emp.status}</span><span className="corporate-specimen-action">{emp.action}</span></div>)}
          </div>
        </aside>
      </section>
      ...
      <section className="corporate-pricing-section corporate-single-details" id="kapasite" aria-labelledby="business-pricing-title">
        <div className="corporate-section-heading">
          ...
          <h2 id="business-pricing-title">Ekip büyüklüğünü seçin. Sistem aynı kalsın.</h2>
          <p>Tüm paketlerde aynı kurumsal yönetim özellikleri bulunur. Değişen yalnızca kullanıcı ve dahil NFC kart sayısıdır.</p>
        </div>
        ...
        <div className="corporate-pack-table-wrap">
          <table className="corporate-pack-table">
            ...
            <thead><tr><th scope="col">Ekip</th><th scope="col">Dahil</th><th scope="col" className="is-num">İlk yıl</th><th scope="col" className="is-num">Yenileme</th><th scope="col"><span className="sr-only">Aksiyon</span></th></tr></thead>
            <tbody>
              {CORPORATE_PACKAGE_LADDER.map((plan) => {
                ...
                return (
                  <tr key={plan.code} className={isRecommended ? "is-recommended" : ""}>
                    <th scope="row"><div className="corporate-table-capacity-wrap"><span>{plan.seats} kişi</span>{isRecommended && <span className="corporate-tier-badge">En Çok Tercih Edilen</span>}</div></th>
                    ...
                    <td className="is-num corporate-price-secondary"><strong>{formatTryFromKurus(renewal)}</strong><span className="corporate-price-period"> / yıl</span></td>
                    <td className="is-action">{corporateCheckoutLive(plan.seats) ? <AddToCartButton productId={CORPORATE_PACKAGE_PRODUCT_SLUG} variantSku={corporatePackageSku(plan.code)} kind="BUSINESS_CARD" name={plan.name} unitPriceKurus={plan.priceKurus} label="Paketi Seç" appearance={isRecommended ? "primary" : "secondary"} className="corporate-plan-text" configuration={{ packageCode: plan.code, seatCount: plan.seats }} /> : <a href={`/kurumsal?plan=${plan.code}#teklif`} className="corporate-plan-text">Teklif Al</a>}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <p className="corporate-pack-note">İlk yıl seçilen kullanıcı sayısı kadar NFC kart, 1 yıllık kurumsal platform ve Türkiye içi kargo dahildir. Yenilemede mevcut kartlar kullanılmaya devam eder; yeni fiziksel kart gönderilmez.</p>
      </section>

      <section className="corporate-use-cases" id="senaryolar" aria-labelledby="corporate-use-cases-title">
        <div className="corporate-section-heading"><span className="section-kicker">NE YÖNETİRSİNİZ?</span><h2 id="corporate-use-cases-title">Kartı değil, çalışan sistemini yönetin.</h2></div>
        <div className="corporate-use-case-grid">{useCases.map((item) => <article key={item.title}><Icon name={item.icon} /><h3>{item.title}</h3><p>{item.text}</p></article>)}</div>
      </section>

      <section className="corporate-steps-section" id="kurulum" aria-labelledby="corporate-steps-title">
        <div className="corporate-section-heading"><span className="section-kicker">KURULUM</span><h2 id="corporate-steps-title">Üç adımda ekibiniz yayında.</h2></div>
        <ol className="corporate-step-grid">{steps.map(([number, title, text]) => <li key={number}><span className="corporate-step-num">{number}</span><h3>{title}</h3><p>{text}</p></li>)}</ol>
      </section>

      <section className="corporate-lead-section" id="teklif" aria-labelledby="corporate-lead-title">
        <div className="corporate-lead-copy"><span className="section-kicker">100+ KİŞİ · ÖZEL KURULUM</span><h2 id="corporate-lead-title">Daha büyük ekipler için birlikte planlayalım.</h2><p>100+ çalışan, özel entegrasyon veya kuruma özel yapılandırma ihtiyacınızı paylaşın. Size uygun kurulum ve fiyatlandırmayla dönüş yapalım.</p><div className="corporate-lead-trust-signals"><span>1 iş günü içinde dönüş</span><span>Ödeme bilgisi istenmez</span><span>İhtiyaca göre yapılandırma</span></div></div>
      ...
      <section className="corporate-final-cta" id="baslayin" aria-labelledby="corporate-final-title">
        <div className="corporate-final-cta__content"><span className="section-kicker">KURUMSAL BAŞLANGIÇ</span><h2 id="corporate-final-title">Ekibiniz için doğru kapasiteyi seçin.</h2><p>Özellik karşılaştırmak yerine yalnızca ekip büyüklüğünüzü belirleyin.</p></div>
        <div className="corporate-final-cta__actions"><a href="#kapasite" className="corporate-cta">Paketleri Gör <span aria-hidden="true">→</span></a><a href="/kurumsal?plan=ENTERPRISE#teklif" className="home-mockup__link-secondary">100+ kişi için teklif al <span aria-hidden="true">→</span></a></div>
      </section>
    </main>
  );
}
