...
function loc(rel){ return fs.readFileSync(path.join(root,rel),'utf8').split(/\r?\n/).length; }
...
const legacyBytes = fs.statSync(path.join(root,'app/legacy-surfaces.css')).size;
legacyBytes <= manifest.budgets.legacyCssMaxBytes ? pass(`legacy CSS did not grow (${legacyBytes}/${manifest.budgets.legacyCssMaxBytes} bytes)`) : fail(`legacy CSS grew to ${legacyBytes} bytes`);

const clientFiles = [...walk(path.join(root,'app'),'.__never__')];
...
const explicitClients = [...collectTs(path.join(root,'app')), ...collectTs(path.join(root,'lib'))]
  .filter(abs => fs.readFileSync(abs,'utf8').trimStart().startsWith('"use client"') || fs.readFileSync(abs,'utf8').trimStart().startsWith("'use client'"));
explicitClients.length <= manifest.budgets.explicitClientBoundaryMax ? pass(`client boundaries did not grow (${explicitClients.length}/${manifest.budgets.explicitClientBoundaryMax})`) : fail(`explicit client boundary count grew to ${explicitClients.length}`);

for (const [rel,max] of Object.entries(manifest.budgets.criticalSurfaceMaxLoc)) {
  if (!fs.existsSync(path.join(root,rel))) { fail(`critical surface missing: ${rel}`); continue; }
  const lines=loc(rel);
