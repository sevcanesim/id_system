...
import { addCartItem, cartAddConflict, cartAddConflictMessage, readCart } from "../../lib/cart";
...
export default function ProductBuy({ variantSku, priceKurus, productSlug, productName }:{variantSku:string;priceKurus:number;productSlug:string;productName:string}) {
 ...
 const add=()=>{const conflict=cartAddConflict(variantSku,readCart()); if(conflict && !window.confirm(cartAddConflictMessage(conflict))) return; addCartItem({productId:productSlug,variantSku,kind:"NFC_PHYSICAL_CARD",name:productName,unitPriceKurus:priceKurus,quantity});setAdded(true);window.setTimeout(()=>setAdded(false),2200)};
 return <div className="yi-buybox">
   <div className="yi-buybox__price"><small>Ana ürün</small><strong>{formatTryFromKurus(priceKurus)}</strong><span>1 fiziksel NFC kart + 1 yıl platform üyeliği dahil</span></div>
