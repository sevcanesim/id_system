...
export const metadata: Metadata = {
  ...
  description: "Kart, kargo, PayTR ödemesi, hesap ve kurumsal kullanım için hızlı ve net yanıtlar.",
};
...
export default async function SupportPage({ searchParams }: { searchParams: Promise<{ q?: string | string[] }> }) {
  ...
  return (
    <main id="main-content" className="support-page">
      <section className="support-hero" aria-labelledby="support-title"><div className="support-hero-inner"><span className="section-kicker">YARDIM MERKEZİ</span><h1 id="support-title">Doğru yanıt,<br />bekletmeden.</h1><p>Kart, profil, sipariş, güvenlik ve kurumsal kullanım için kısa, net ve işe yarayan cevaplar.</p><form className="support-search" role="search" action="/destek" method="get"><span className="support-search__icon" aria-hidden="true"><Icon name="search" /></span><input name="q" type="search" defaultValue={query} placeholder="Örn. kayıp kart, kargo, ödeme…" aria-label="Yardım merkezinde ara" enterKeyHint="search" /><button type="submit">Yanıtı bul</button></form></div></section>
      <section className="support-main" aria-labelledby="support-topics-title"><div className="support-topics"><div className="support-section-head support-section-head--compact"><div><span className="section-kicker">KONULAR</span><h2 id="support-topics-title">Neyi netleştirmek istersin?</h2></div></div><nav className="support-topic-grid" aria-label="Yardım konuları">{topics.map((topic) => <Link className="support-topic-card" key={topic.id} href={`#konu-${topic.id}`}><TopicIcon value={topic.icon} /><div><h3>{topic.title}</h3><p>{topic.text}</p></div><span className="support-topic-arrow" aria-hidden="true">→</span></Link>)}</nav></div>
      ...
      <section className="support-contact" aria-label="Doğrudan destek"><div><span className="section-kicker">DOĞRUDAN DESTEK</span><h2>Hâlâ yanıtını bulamadın mı?</h2></div><p>Sipariş veya kart kodunla bize yaz. Hesabın varsa paneline doğrudan dönebilirsin.</p><div className="support-contact-actions"><a href={`mailto:${legal.email}`}>Desteğe yaz <span aria-hidden="true">→</span></a><Link href="/giris">Hesabıma dön</Link></div></section>
    </main>
  );
}
