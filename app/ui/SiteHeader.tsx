...
export default function SiteHeader({theme="light", actions=[]}:{theme?: "dark"|"light";actions?:Array<{href:string;label:string;primary?:boolean}>}) {
  ...
  return <header className={`yi-header yi-header--${theme}`}>
    <div className="yi-container yi-header__inner">
      ...
      <div className="yi-header__actions">
        <Link className="yi-cart" href="/sepet" aria-label={count?`Sepet, ${count} ürün`:"Sepet"}>Sepet{count>0&&<b>{count}</b>}</Link>
        {(actions.filter(a=>a.primary).slice(0,1)[0] ? actions.filter(a=>a.primary).slice(0,1) : [{href:"/urunler/nfc-kart",label:"NFC Kartı İncele",primary:true}]).map(a=><ButtonLink key={a.href} href={a.href} variant="primary">{a.label}</ButtonLink>)}
        <ButtonLink href={signedIn?"/hesabim":"/giris"} variant="secondary">{signedIn?"Hesabım":"Giriş Yap"}</ButtonLink>
        <button className="yi-menu" type="button" aria-label={open?"Menüyü kapat":"Menüyü aç"} aria-expanded={open} onClick={()=>setOpen(v=>!v)}><span/><span/><span/></button>
      </div>
    </div>
  </header>;
}
