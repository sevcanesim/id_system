...
check(sessionRoute.includes("export async function GET") && sessionRoute.includes("refreshToken"), "legacy session bridge remains available for OAuth and recovery compatibility");
...
check(browserClient.includes('fetch("/api/auth/session"') && browserClient.includes("setSession"), "OAuth and recovery compatibility can restore the in-memory session");
