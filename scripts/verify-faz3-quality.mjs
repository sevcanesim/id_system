...
if (hasE2eSpecs()) {
  const responsive = fs.readFileSync("tests/e2e/responsive-master.spec.ts", "utf8");
  const visual = fs.readFileSync("tests/e2e/visual-layout-audit.spec.ts", "utf8");
  const requiredWidths = [320, 360, 375, 390, 768, 1024, 1280, 1440];
  for (const width of requiredWidths) {
    pass(new RegExp(`width:\\s*${width}\\b`).test(responsive), `responsive contract includes ${width}px`);
  }
  pass(responsive.includes("document overflow") && responsive.includes("topLevelMasks"), "responsive suite reports real horizontal overflow instead of masking it");
  pass(visual.includes("mobile-375") && visual.includes("mobile-390") && visual.includes("document horizontal overflow"), "visual layout suite covers mobile phone widths and overflow");
  pass(visual.includes("home-package-matrix__mobile-cards"), "home mobile package comparison has a regression assertion");
} else {
...
pass(/styles\.notice/.test(cards) && /EmptyState/.test(states), "individual cards surface uses the canonical empty-state treatment");
...
pass(canonical.includes(".id-sidebar__user-name"), "corporate sidebar identity text remains explicit");
