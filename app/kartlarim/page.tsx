...
export default function MyCardsPage(){
 ...
 return <DashboardShell title={primary?.name?`Merhaba, ${primary.name.split(" ")[0]}`:"Yenomi ID'n"} description="Dijital kimliğini, kartlarını ve hesabını tek çalışma alanından yönet.">
  {!primary?<div className="yi-empty-app"><span>İLK ADIM</span><h2>Kimliğin burada başlıyor.</h2><p>{spare?"Edinilmiş bir hizmet hakkın var. Profilini oluşturarak kartını hazırla.":"Bir Yenomi ID ürünü edinerek dijital kimliğini aktive et."}</p><ButtonLink href={spare?"/olustur":"/urunler/nfc-kart"} variant="primary">{spare?"Kimliğimi oluştur":"NFC Kartı Satın Al"}</ButtonLink></div>:
