...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!order || (order.activation_deadline_at && new Date(order.activation_deadline_at) < new Date())) return NextResponse.json({ ok: true, message: "Uygun bir sipariş bulunursa yeni bağlantı gönderildi." });
