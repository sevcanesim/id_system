...
export default function NetworkingCapture({ profileId, profileName, visitorId, eventId, source, locale }: NetworkingCaptureProps) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/networking/leads", {
        ...
        body: JSON.stringify({ profileId, visitorId, eventId, source, locale: resolvedLocale, requestMeeting: false, fullName: form.fullName, email: form.email, phone: form.phone, company: form.company, position: form.position, interests: [], introduction: "" }),
      });
  ...
  if (mode === "success") return (
    <section className="p12-networking" aria-live="polite">
      <div className="p12-networking-success"><div className="p12-networking-success-mark" aria-hidden="true">✓</div><div><p className="p12-networking-kicker">{t.successTitle}</p><p className="p12-networking-copy">{t.successBody(ownerName)}</p></div></div>
  ...
  if (mode === "share") return (
    <section className="p12-networking">
      <div className="p12-networking-heading"><p className="p12-networking-kicker">{t.eyebrow}</p><p className="p12-networking-copy">{t.intro}</p></div>
      <form className="p12-networking-form" onSubmit={submit}>
        ...
        {showProfessional ? <div className="p12-networking-professional"><label className="p12-networking-field"><span>{t.company}</span><input maxLength={120} value={form.company} onChange={(event) => update("company")(event.target.value)} /></label><label className="p12-networking-field"><span>{t.position}</span><input maxLength={120} value={form.position} onChange={(event) => update("position")(event.target.value)} /></label></div> : null}
