...
function normalizeShippingAddress(value: unknown): ShippingAddress | null {
  if (Array.isArray(value)) {
    const firstValid = value.find(isShippingAddress);
...
export default function MyOrdersPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (cancelled) return;
        if (!response.ok) {
          setMessage(safeClientMessage(result, "Sipariş bilgileri şu anda yüklenemiyor. Lütfen yeniden dene."));
        } else {
  ...
  if (loading) {
    return (
      <UserPanelShell activeKey="orders" title="Siparişlerim" description="Sipariş bilgileriniz yükleniyor.">
        <Card><p className="p9-section-copy">Siparişler yükleniyor…</p></Card>
      </UserPanelShell>
    );
  }
  ...
  if (!signedIn) {
    return (
      <UserPanelShell activeKey="orders" title="Siparişlerim" description="Siparişlerinizi görüntülemek için hesabınıza giriş yapın.">
        <EmptyState
          title="Oturum gerekli"
          description="Siparişleriniz hesabınıza bağlandığında burada görünür."
          action={<ButtonLink href="/giris?next=%2Fsiparislerim" variant="primary">Hesabına gir</ButtonLink>}
        />
      </UserPanelShell>
    );
  }
  ...
  return (
    <UserPanelShell
      ...
      description={orders.length
        ? `${orders.length} sipariş · ${activeCount} devam eden süreç`
        : "Ürün, ödeme, hazırlık ve kargo durumunu tek yerden takip edin."}
    >
      <section className="p9-stack">
        ...
        <div className="p9-order-list">
          {orders.length === 0 ? (
            <EmptyState
              title="İlk kartın henüz yok."
              description="NFC + QR kartını al; 1 yıl dijital hizmet ve Türkiye içi kargo dahil."
              action={<ButtonLink href="/urunler/nfc-kart" variant="primary">NFC Kartı Satın Al</ButtonLink>}
            />
          ) : orders.map((order) => {
            ...
            const isTerminalException = ["CANCELLED", "REFUNDED"].includes(order.status);

            return (
              <Card className="p9-order" data-tone={info.tone === "success" ? "success" : undefined} key={order.id}>
                <div className="p9-order__head">
                  <div>
                    <span className="p9-order__date">
                      {new Date(order.created_at).toLocaleDateString("tr-TR", { day: "2-digit", month: "long", year: "numeric" })}
                    </span>
                    <h2 className="p9-order__number">{order.order_number}</h2>
                    <p className="p9-order__products">
                      {order.commerce_order_items.map((item) => `${item.quantity} × ${item.product_name}`).join(" · ")}
                    </p>
                  </div>
                ...
                {!isTerminalException && (
                  <div className={`p9-order__progress ${styles.progress}`} aria-label={`Sipariş durumu: ${info.label}`}>
                    {[1, 2, 3, 4, 5].map((step) => (
                      <span key={step} data-active={info.step >= step ? "true" : "false"} />
                    ))}
                  </div>
                )}

                <p className="p9-order__status">{info.description}</p>
                <div className="p9-order__meta">
                  ...
                  <div><small>Ödeme</small><strong>{order.paid_at ? new Date(order.paid_at).toLocaleDateString("tr-TR") : "Bekleniyor"}</strong></div>
                  <div><small>Teslimat</small><strong>{address ? `${address.district}, ${address.city}` : "—"}</strong></div>
                </div>

                {order.status === "SHIPPED" && (
                  <div className="p9-order__shipping">
                    <strong>Kargo</strong>
                    <span>{order.tracking_company || "Kargo firması"}{order.tracking_number ? ` · ${order.tracking_number}` : ""}</span>
                  </div>
                )}
              </Card>
            );
