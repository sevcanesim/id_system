...
export default function AdminPage() {
  ...
  async function load() {
    try { const response = await fetch("/api/admin/commerce/orders", { credentials: "same-origin", cache: "no-store" }); const result = await response.json(); if (response.status === 403) setAuthorized(false); else if (!response.ok) setMessage(result.error ?? "Siparişler yüklenemedi."); else { setOrders(result.orders ?? []); setTracking(Object.fromEntries((result.orders ?? []).map((order: Order) => [order.id, { company: order.tracking_company ?? "", number: order.tracking_number ?? "" }]))); } } catch { setMessage("Sunucuya ulaşılamadı."); } finally { setLoading(false); }
  }
  useEffect(() => { void load(); }, []);

  async function updateOrder(orderId: string, status: Status) {
    setSaving(orderId); setMessage(""); const t = tracking[orderId] ?? { company: "", number: "" };
    try { const response = await fetch("/api/admin/commerce/orders", { method: "PATCH", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId, action: "UPDATE_ORDER", status, trackingCompany: t.company || null, trackingNumber: t.number || null }) }); const result = await response.json(); if (!response.ok) setMessage(result.error ?? "Sipariş güncellenemedi."); else await load(); } catch { setMessage("Güncelleme sırasında sunucuya ulaşılamadı."); } finally { setSaving(null); }
  }
  async function resendActivation(orderId: string) {
    setSaving(orderId); setMessage("");
    try { const response = await fetch("/api/admin/commerce/orders", { method: "PATCH", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId, action: "RESEND_ACTIVATION" }) }); const result = await response.json(); setMessage(response.ok ? (result.message ?? "Aktivasyon bağlantısı yeniden gönderildi.") : (result.error ?? "Aktivasyon bağlantısı gönderilemedi.")); } catch { setMessage("Aktivasyon gönderilirken sunucuya ulaşılamadı."); } finally { setSaving(null); }
  }

  async function loadReconciliation() { setReconciliationLoading(true); setReconciliationMessage(""); try { const response = await fetch("/api/admin/commerce/reconciliation", { credentials: "same-origin", cache: "no-store" }); const result = await response.json(); if (!response.ok) setReconciliationMessage(result.error ?? "Ödeme mutabakatı yüklenemedi."); else { setReconciliationRows(result.rows ?? []); setReconciliationSummary(result.summary ?? { checkedOrders: 0, requiresReview: 0, openFulfillmentIssues: 0, orphanPaidAttempts: 0 }); } } catch { setReconciliationMessage("Ödeme mutabakatı için sunucuya ulaşılamadı."); } finally { setReconciliationLoading(false); } }
  useEffect(() => { if (tab === "reconciliation" && reconciliationRows.length === 0 && !reconciliationLoading) void loadReconciliation(); }, [tab]);
  async function resolveReconciliationIssue(issueId: string) { const note = (resolutionNotes[issueId] ?? "").trim(); if (note.length < 8) { setReconciliationMessage("Çözüm notu en az 8 karakter olmalı."); return; } setResolvingIssue(issueId); try { const response = await fetch("/api/admin/commerce/reconciliation", { method: "PATCH", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ issueId, resolutionNote: note }) }); const result = await response.json(); if (!response.ok) setReconciliationMessage(result.error ?? "Mutabakat kaydı çözümlenemedi."); else { setReconciliationMessage("Mutabakat kaydı çözümlendi."); await loadReconciliation(); } } finally { setResolvingIssue(null); } }
  async function runPaidReconciliation() { setReconcilingPaid(true); try { const response = await fetch("/api/admin/commerce/reconciliation", { method: "POST", credentials: "same-origin" }); const result = await response.json(); setReconciliationMessage(response.ok ? "Kayıtlı PAID siparişler onarıldı." : (result.error ?? "Onarım çalıştırılamadı.")); await loadReconciliation(); } finally { setReconcilingPaid(false); } }

  async function loadCorporate() { setCorporateLoading(true); try { const response = await fetch("/api/admin/organizations", { credentials: "same-origin" }); const result = await response.json(); if (!response.ok) setCorporateMessage(result.error ?? "Kurumsal hesaplar yüklenemedi."); else { setAccounts(result.accounts ?? []); setPlans(result.plans ?? []); } } finally { setCorporateLoading(false); } }
  useEffect(() => { if (tab === "corporate" && accounts.length === 0 && !corporateLoading) void loadCorporate(); }, [tab]);
  async function provisionOrganization(event: FormEvent) { event.preventDefault(); setProvisioning(true); setCorporateMessage(""); try { const response = await fetch("/api/admin/organizations", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create_tenant", name: form.name, taxNumber: form.taxNumber, taxOffice: form.taxOffice, legalAddress: form.legalAddress, city: form.city, planCode: form.planCode, employeeLimit: form.employeeLimit ? Number(form.employeeLimit) : undefined, digitalCardLimit: form.digitalCardLimit ? Number(form.digitalCardLimit) : undefined, physicalCardLimit: form.physicalCardLimit ? Number(form.physicalCardLimit) : undefined, mailCreditLimit: form.mailCreditLimit ? Number(form.mailCreditLimit) : undefined, billingPeriod: form.billingPeriod, termDays: form.termDays ? Number(form.termDays) : undefined, status: form.status }) }); const result = await response.json(); if (!response.ok) setCorporateMessage(result.error ?? "Şirket oluşturulamadı."); else { setCorporateMessage(`${result.organization.name} oluşturuldu.`); await loadCorporate(); } } finally { setProvisioning(false); } }
  async function attachManager(organizationId: string) { const fields = attachForm[organizationId]; if (!fields?.email || !fields.fullName) { setCorporateMessage("Yönetici e-postası ve adı gerekli."); return; } setProvisioning(true); try { const response = await fetch("/api/admin/organizations", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "attach_manager", organizationId, email: fields.email, fullName: fields.fullName, role: fields.role || "OWNER" }) }); const result = await response.json(); setCorporateMessage(response.ok ? `${fields.email} şirkete bağlandı.` : (result.error ?? "Yönetici bağlanamadı.")); if (response.ok) await loadCorporate(); } finally { setProvisioning(false); } }
  async function setOrganizationStatus(organizationId: string, status: "ACTIVE" | "SUSPENDED") { const response = await fetch("/api/admin/organizations", { method: "PATCH", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ organizationId, status }) }); const result = await response.json(); if (!response.ok) setCorporateMessage(result.error ?? "Durum güncellenemedi."); else await loadCorporate(); }
